...
def init_kernels(win_len, win_inc, fft_len, win_type=None, invers=False):
    if win_type == 'None' or win_type is None:
        ...
        if win_type == "hanning sqrt":
            window = get_window("hann", win_len, fftbins=True)  # win_len
...
class ConviSTFT(nn.Module):
    ...
    def forward(self, inputs, phase=None):
        ...
        outputs = torchF.conv_transpose1d(inputs, self.weight, stride=self.stride)

        # this is from torch-stft: https://github.com/pseeth/torch-stft
        t = self.window.repeat(1, 1, inputs.size(-1)) ** 2
        coff = torchF.conv_transpose1d(t, self.enframe, stride=self.stride)
...
if __name__ == '__main__':
    # stft = ConvSTFT(400, 100, 512, "hanning", 'complex')
    stft = ConvSTFT(512, 128, 512, "hanning", 'complex')
    # istft = ConviSTFT(400, 100, 512, "hanning", 'complex')
    istft = ConviSTFT(512, 128, 512, "hanning", 'complex')
    test_inp = torch.randn(2, 512)
    stft_ = stft(test_inp)
    print(stft_.size())
    out = istft(stft_)
