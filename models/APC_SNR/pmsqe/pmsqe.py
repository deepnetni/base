...
class SingleSrcPMSQE(nn.Module):
    ...
    def forward(self, est_targets, targets, pad_mask=None):
        ...
        """
        assert est_targets.shape == targets.shape, str(est_targets.shape) + str(targets.shape)
        ...
        dims = [-1, -2]
        # print(torch.mean(wd_frame),torch.mean(wda_frame))
        if vad_mask is not None:
            pmsqe_frame = (self.alpha * wd_frame + self.beta * wda_frame) * vad_mask
        else:
    ...
    def compute_distortion_tensors(self, ref_bark_spec, deg_bark_spec):
        ...
        # print(torch.mean(r))
        # drawer.plot_mesh(r[0].data.numpy(), "r:" + str(torch.mean(r).data))
        ...
        # print(torch.mean(sym_d))
        # drawer.plot_mesh(sym_d[0].data.numpy(), "sym_d:" + str(torch.mean(sym_d).data))
...
if __name__ == '__main__':
    test_est = torch.tensor(np.load(r"F:\python programes\interphone_tensorflow\wav/test_est.npy"), dtype=torch.float32)
    test_label = torch.tensor(np.load(r"F:\python programes\interphone_tensorflow\wav/test_label.npy"),
                              dtype=torch.float32)
    pmsqe = SingleSrcPMSQE()
    print(pmsqe(test_est ** 2, test_label ** 2))
    print(torch.mean(pmsqe(test_est ** 2, test_label ** 2)))
