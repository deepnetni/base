...
import numpy as np
import models.APC_SNR.constant as constant
...
class AcousticScaler(nn.Module):
    def __init__(self, theta=0.01, eps=1.0, mag_bins=256, no_grad=True):
        ...
        self.register_buffer(
            "zwicker_power",
            torch.tensor(
                constant.get_zwicker_power_to_nftt(mag_bins), dtype=torch.float
            )
            .unsqueeze(0)
            .unsqueeze(0),
        )
    ...
    def forward(self, est, clean=None):
        ...
        if self.no_grad:
            with torch.no_grad():
                ...
                clean_scales = (clean_power + self.eps) ** (
                    (self.zwicker_power - 1) * 0.5
                )
                return double_size(est_scales, need_pad).clamp_(
                    self.theta, 1
                ), double_size(clean_scales, need_pad).clamp_(self.theta, 1)
        else:
            est = est.permute(0, 2, 1)  # B,T,2*F
            clean = clean.permute(0, 2, 1)
            est_real, est_imag = torch.chunk(est, 2, dim=-1)
            ...
            return double_size(est_scales, need_pad).clamp_(self.theta, 1), double_size(
                clean_scales, need_pad
            ).clamp_(self.theta, 1)
...
def double_size(inp, need_pad):
    ...
    if need_pad != 0:
        return (
            torch.nn.functional.pad(inp, [0, need_pad, 0, 0], value=1e-8)
            .repeat(1, 1, 2)
            .permute(0, 2, 1)
        )
    else:
