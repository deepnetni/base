...
def cumulative_normalization(
    original_signal_mag: Tensor, sliding_window_len: int = 192
) -> Tensor:
    ...
    for frame_idx in range(frame_num):
        if frame_idx < sliding_window_len:
            ...
            mu = alp * mu + (1 - alp) * torch.mean(
                original_signal_mag[:, frame_idx, :], dim=-1
            ).reshape(batch_size, 1)
        else:
            current_frame_mu = torch.mean(
                original_signal_mag[:, frame_idx, :], dim=-1
            ).reshape(batch_size, 1)
    ...
    XrMM = (
        ...
        .permute(0, 2, 1)
        .reshape(batch_size, frame_num, 1, 1)
    )
    return XrMM
...
class McNetIO(Module):
    ...
    def prepare_input(self, x: Tensor, *args, **kwargs) -> Dict[str, Any]:
        ...
        X = torch.stft(
            x,
            n_fft=self.ft_len,
            hop_length=self.ft_hop,
            window=self.window,
            win_length=self.ft_len,
            return_complex=True,
        )  # type:ignore
        X = X.reshape(
            (batch_size, chn_num, X.shape[-2], X.shape[-1])
        )  # (batch, channel, freq, time frame)
        ...
        if self.use_cumulative_normalization == False:
            XrMM = (
                torch.abs(Xr).mean(dim=(1, 2)).reshape(batch_size, 1, 1, 1)
            )  # Xr_magnitude_mean: mean of the magnitude of the ref channel of X
        else:
            XrMM = cumulative_normalization(
                original_signal_mag=torch.abs(Xr),
                sliding_window_len=self.sliding_window_len,
            )

        X /= XrMM + 1e-8
        input = torch.view_as_real(X).reshape(
            batch_size, frame_num, freq_num, chn_num * 2
        )
        ...
        return {
            ...
            "XrMag": XrMag,
            "XMag": XMag,
            "original_time_len": time,
        }

    def prepare_target(
        self, x: Tensor, yr: Tensor, XrMM: Tensor, *args, **kwargs
    ) -> Any:
    ...
    def prepare_prediction(
        ...
        **kwargs,
        ...
        output = torch.view_as_complex(
            output.reshape(batch_size, frame_num, freq_num, 2)
        )  # [B,T,F]
        ...
        pred = torch.istft(
            ...
            window=self.window,
            win_length=self.ft_len,
            length=original_time_len,
        )  # [B, t]  type:ignore
        return pred

    def prepare_time_domain(
        self,
        x: Tensor,
        prediction: Tensor,
        XrMM: Tensor,
        ...
        **kwargs,
        ...
        return prediction * XrMM.reshape(
            XrMM.shape[0], 1
        )  # 此处再乘上系数，prepare_prediction处乘上XrMM会使得不同的句子具有不同的权重
...
class CCIO(McNetIO):
    def prepare_target(
        ...
        target = torch.stft(
            ...
            win_length=self.ft_len,
            return_complex=True,
        )  # type:ignore # (batch, freq, time frame)
        target /= (
            XrMM.reshape(XrMM.shape[0], 1, 1) + 1e-8
        )  # normalization as prediction, (batch, freq, time frame)
        return target
    ...
    def prepare_prediction(
        ...
        **kwargs,
        ...
        """prepare prediction from the output of network for loss function"""
        batch_size, frame_num, freq_num, chn_num = X.shape
        output = output.reshape(batch_size, frame_num, freq_num, 2).permute(
            0, 2, 1, 3
        )  # [B,F,T,2]
...
class cIRMIO(McNetIO):
    def prepare_target(
        ...
        target = get_complex_ideal_ratio_mask(
            noisy_complex_tensor=stft_xr, clean_complex_tensor=stft_yr
        )
        ...
        for act_func in act_funcs:
            if act_func == "SiLU" or act_func == "swish":
                ...
                raise NotImplementedError(
                    f"Not implemented activation function {act_func}"
                )
...
class McNet(pl.LightningModule):
    ...
    def on_train_start(self):
        rank_zero_info(
            "order: "
            + str(self.hparams.order)
            + ", sub_freqs: "
            + str(self.hparams.sub_freqs)
        )
        if self.current_epoch == 0:
            if (
                self.trainer.is_global_zero
                and hasattr(self.logger, "log_dir")
                and "notag" not in self.hparams.exp_name
            ):
                ...
                tag_and_log_git_status(
                    self.logger.log_dir + "/git.out",
                    self.logger.version,
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Dict[str, Any]]:  # type:ignore
        ...
        for curr in self.order:
            if self.use_dense_net:
                ...
                B, T, F, H = i.shape
            if (
                not curr.startswith("sub2")
                and not curr.startswith("sub3")
                and not curr.startswith("full4freq")
            ):
            ...
            if curr == "freq":  # 沿频率的序列，[BT, F, H]
                i = i.reshape(B * T, F, -1)
                i = self.freq(i).reshape(
                    B, T, F, -1
                )  # type:ignore # to shape [B,T,F,H]
                ...
                i = i.permute(0, 1, 3, 2).reshape(
                    B * T, -1, F, 1
                )  # to shape [BT, H, F, 1], 1 is for unfold module (4D supported only)
                i = torch.concat(
                    [
                        i[:, :, : self.hparams.sub_freqs, :],
                        i,
                        i[:, :, -self.hparams.sub_freqs :, :],
                    ],
                    dim=2,
                )  # type:ignore # freqs: -3 -2 -1 0 ... 256 0 1 2
                i = torch.nn.functional.unfold(
                    i, kernel_size=(self.hparams.sub_freqs * 2 + 1, 1)
                )  # type:ignore # shape [BT, H'=H*2S+1, F]
                ...
                if curr.endswith("+X"):
                    X = (
                        data["input"].permute(0, 1, 3, 2).reshape(B * T, -1, F, 1)
                    )  # to shape [BT, H, F, 1], 1 is for unfold module (4D supported only)
                else:
                    assert curr.endswith("+XrMag"), curr
                    X = (
                        data["XrMag"].permute(0, 1, 3, 2).reshape(B * T, -1, F, 1)
                    )  # to shape [BT, H, F, 1], 1 is for unfold module (4D supported only)
                if self.hparams.sub_freqs != 0:
                    X = torch.concat(
                        [
                            X[:, :, : self.hparams.sub_freqs, :],
                            X,
                            X[:, :, -self.hparams.sub_freqs :, :],
                        ],
                        dim=2,
                    )  # type:ignore # freqs: -3 -2 -1 0 ... 256 0 1 2
                    Xsub = torch.nn.functional.unfold(
                        X, kernel_size=(self.hparams.sub_freqs * 2 + 1, 1)
                    )  # type:ignore # shape [BT, H'=H*2S+1, F]，原始信号拼接的子带
                    i = i.permute(0, 1, 3, 2).reshape(
                        B * T, -1, F
                    )  # to shape [BT, H, F]
                else:
                ...
                if self.hparams.sub_freqs[0] != 0:
                    ...
                    Xsub = torch.nn.functional.unfold(
                        X, kernel_size=(self.hparams.sub_freqs[0] * 2 + 1, 1)
                    )  # type:ignore # shape [BT, H'=H*2S+1, F]，原始信号拼接的子带
                else:
                    Xsub = X.reshape(B * T, -1, F)
                i = i.permute(0, 1, 3, 2).reshape(
                ...
                i = torch.nn.functional.unfold(
                    i, kernel_size=(self.hparams.sub_freqs[1] * 2 + 1, 1)
                )
                i = torch.concat([i, Xsub], dim=1)
                i = i.reshape(B, T, -1, F).permute(0, 3, 1, 2).reshape(B * F, T, -1)
                i = self.sub(i)  # type:ignore
                ...
                if curr.endswith("-allch"):
                    i = data["XMag"].permute(
                        0, 2, 3, 1
                    )  # [B,F,H,T] here H=the number of channels
                    B, F, H, T = i.shape
                    i = torch.nn.functional.pad(
                        i,
                        pad=self.hparams.look_past_and_ahead,
                        mode="constant",
                        value=0,
                    )
                    i = i.reshape(
                        B * F * H, 1, -1, 1
                    )  # unfold module (4D supported only)
                    i = torch.nn.functional.unfold(
                        i,
                        kernel_size=(
                            self.hparams.look_past_and_ahead[0]
                            + self.hparams.look_past_and_ahead[1]
                            + 1,
                            1,
                        ),
                    )  # [B*F*H,H'=(T_look_ahead+T_look_past+1)*1,T]
                    i = (
                        i.reshape(B, F, H, -1, T)
                        .permute(0, 2, 4, 1, 3)
                        .reshape(B * H * T, F, -1)
                    ...
                    )  # [B*F,H'=(T_look_ahead+T_look_past+1)*H,T]
                    i = (
                        i.reshape(B, F, -1, T).permute(0, 3, 1, 2).reshape(B * T, F, -1)
                    )  # [B*T,F,H']
                    i = self.full(i).reshape(B, T, F, -1)  # type:ignore
            elif curr.startswith("fullfreq"):
                i = i.reshape(B * T, F, -1)
                i = self.full(i).reshape(
                    B, T, F, -1
                )  # type:ignore # to shape [B,T,F,H]
                ...
                if curr.endswith("+XrMag"):
                    ...
                    XrMag = torch.nn.functional.unfold(
                        XrMag,
                        kernel_size=(
                            self.hparams.look_past_and_ahead[0]
                            + self.hparams.look_past_and_ahead[1]
                    ...
                    )  # [B*F,H'=(T_look_ahead+T_look_past+1)*1,T]
                    XrMag = (
                        XrMag.reshape(B, F, -1, T)
                        .permute(0, 3, 1, 2)
                        .reshape(B * T, F, -1)
                    )  # [B*T,F,H']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            "val/" + self.io.loss_name, loss, batch_size=x.shape[0], sync_dist=True
        )
    ...
    def test_epoch_end(self, results):
        ...
        if self.trainer.is_global_zero:
            ...
            path = os.path.join(
                self.trainer.logger.log_dir, "results_{}.json".format(dtstr)
            )
            ...
            df.mean(numeric_only=True).to_json(
                os.path.join(self.trainer.logger.log_dir, "results_mean.json"), indent=4
            )
    ...
    def test_step(self, batch, batch_idx):
        ...
        wav_name = (
            paras["wav_name"][0]
            if "wav_name" in paras
            else str(paras["index"].cpu().item()) + ".wav"
        )
        sr = paras["sr"][0]
        result_dict = {
            "id": batch_idx,
            "wav_name": wav_name,
            self.io.loss_name: loss.item(),
        }
        ...
        metrics, input_metrics, imp_metrics = cal_metrics_functional(
            ...
            preds=yr_hat,
            target=yr,
            original=x[:, self.io.ref_chn_idx, :],
            fs=sr.item(),
        )
    ...
    def predict_step(
        ...
        x = batch
        # 创建保存目录
        # self.exp_save_path = os.path.join(
        #     self.trainer.logger.log_dir, paras["noisy_dataset_file_name"][0]
        # )
        # os.makedirs(self.exp_save_path, exist_ok=True)
