...
def build_rnn_block(
    ...
    if (rnn_type.lower() == "qrnn") and QRNN is not None:
        if bidirectional:
            ...
            rnn_size = 2 * rnn_size
        rnn = QRNN(
            in_size, rnn_size, rnn_layers, dropout=dropout, window=2, use_cuda=use_cuda
        )
    elif rnn_type.lower() == "lstm" or rnn_type.lower() == "gru":
        rnn = getattr(nn, rnn_type.upper())(
            in_size, rnn_size, rnn_layers, dropout=dropout, bidirectional=bidirectional
        )
    else:
...
class Saver(object):
    ...
    def load_pretrained_ckpt(
        ...
        st_dict = torch.load(
            ckpt_file, map_location=lambda storage, loc: storage.cuda(0)
        )
...
class GDeconv1DBlock(NeuralBlock):
    def __init__(
        ...
        self.deconv = nn.ConvTranspose1d(
            ninp, Wfmaps, kwidth, stride=stride, padding=pad, bias=bias
        )
    ...
    def forward(self, x):
        ...
        if (self.stride % 2 != 0 and self.kwidth % 2 == 0) or (
            self.stride % 2 == 0 and self.kwidth % 2 != 0
        ):  # and self.stride > self.kwidth:
            h = h[:, :, :-1]
        h = forward_norm(h, self.norm)
        h = forward_activation(self.act, h)
        return h
...
class ResBasicBlock1D(NeuralBlock):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv1d(
            inplanes, planes, kwidth, stride=1, padding=P, bias=False, dilation=dilation
        )
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv1d(
            planes, planes, kwidth, padding=P, dilation=dilation, bias=False
        )
...
class ResDilatedModule(NeuralBlock):
    def __init__(
        ...
        self.conv_1x1_res_norm = build_norm_layer(
            norm_type, self.conv_1x1_res, res_fmaps
        )
...
class SincConv(nn.Module):
    def __init__(
        self, N_filt, Filt_dim, fs, stride=1, padding="VALID", pad_mode="reflect"
    ):
    ...
    def forward(self, x):
        ...
        for i in range(self.N_filt):
            low_pass1 = (
                ...
                * sinc(filt_beg_freq[i].float() * self.freq_scale, t_right, cuda)
            )
            low_pass2 = (
                2
                * filt_end_freq[i].float()
                * sinc(filt_end_freq[i].float() * self.freq_scale, t_right, cuda)
            )
        ...
        if self.padding == "SAME":
            if self.stride > 1:
                x_p = F.pad(
                    x, (self.Filt_dim // 2 - 1, self.Filt_dim // 2), mode=self.pad_mode
                )
            else:
                x_p = F.pad(
                    x, (self.Filt_dim // 2, self.Filt_dim // 2), mode=self.pad_mode
                )
        else:
            x_p = x
        out = F.conv1d(
            x_p, filters.view(self.N_filt, 1, self.Filt_dim), stride=self.stride
        )
        return out
...
class SincConv_fast(nn.Module):
    ...
    def forward(self, waveforms):
        ...
        high = torch.clamp(
            low + self.min_band_hz + torch.abs(self.band_hz_).to(waveforms.device),
            self.min_low_hz,
            self.sample_rate / 2,
        )
        band = (high - low)[:, 0]
    ...
    def forward(self, x):
        if self.kwidth > 1 and not self.sincnet:
            ...
            if self.stride > 1 or self.kwidth % 2 == 0:
                if self.dilation > 1:
                    raise ValueError(
                        "Cannot make dilated convolution with " "stride > 1"
                    )
                P = (self.kwidth // 2 - 1, self.kwidth // 2)
            else:
...
class VQEMA(nn.Module):
    ...
    def __init__(self, emb_K, emb_dim, beta, gamma, eps=1e-5):
        super().__init__()
        self.emb_K = emb_K
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            self.ema_cluster_size = (
                (self.ema_cluster_size + self.eps) / (n + self.emb_K * self.eps) * n
            )
