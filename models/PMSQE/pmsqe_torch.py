...
class PMSQE(nn.Module):
    ...
    def __init__(
        self, window_name="sqrt_hann", window_weight=1.0, bark_eq=True, gain_eq=True
    ):
        ...
        self.abs_thresh_power = nn.Parameter(
            Tensor(
                [
                    51286152.00,
                    2454709.500,
        ...
        self.modified_zwicker_power = nn.Parameter(
            Tensor(
                [
                    0.25520097857560436,
                    0.25520097857560436,
        ...
        self.width_of_band_bark = nn.Parameter(
            Tensor(
                [
                    0.157344,
                    0.317994,
        ...
        params_16k = Path(__file__).resolve().parent / "bark_matrix_16k.mat"

        self.bark_matrix = nn.Parameter(
            # Tensor(loadmat("bark_matrix_16k.mat")["Bark_matrix_16k"]),
            Tensor(loadmat(params_16k)["Bark_matrix_16k"]),
            requires_grad=False,
        )

        mask_sll = np.zeros(shape=[int(self.nbins / 2 + 1)], dtype=np.float32)
        mask_sll[11] = 0.5 * 25.0 / 31.25
        mask_sll[12:104] = 1.0
        mask_sll[104] = 0.5
        mask_sll = (
            mask_sll
            * self.pow_correc_factor
            * (self.nbins + 2.0)
    ...
    def forward(self, output, target, pad_mask):
        ...
        :param pad_mask:  Dimensions (B,T,1).
            Mask to indicate the padding frames (1 indicates valid frame and 0 padding). Float tensor.
        ...
        if self.bark_eq:
            deg_bark_spectra = self.bark_frequency_equalization(
                ref_bark_spectra, deg_bark_spectra
            )

        if self.gain_eq:
            deg_bark_spectra = self.bark_gain_equalization(
                ref_bark_spectra, deg_bark_spectra
            )
    ...
    def loudness_computation(self, bark_spectra):
        ...
        loudness_dens = (
            self.Sl
            * torch.pow(self.abs_thresh_power / 0.5, self.modified_zwicker_power)
            * (
                torch.pow(
                    0.5 + 0.5 * bark_spectra / self.abs_thresh_power,
        ...
        loudness_dens_limited = torch.where(
            cond, torch.zeros_like(loudness_dens), loudness_dens
        )
    ...
    def per_frame_distortion(self, D, DA, total_power_ref):
        ...
        D_frame = (
            torch.sqrt(
                torch.sum(
                    torch.pow(D * self.width_of_band_bark, 2.0), dim=-1, keepdim=True
                )
            )
            * self.sqrt_total_width
        )
