...
class SingleSrcPMSQE(nn.Module):
    ...
    def forward(self, est_targets, targets, pad_mask=None):
        ...
        if self.bark_eq:
            deg_bark_spectra = self.bark_freq_equalization(
        ...
        sym_d, asym_d = self.compute_distortion_tensors(
            ref_bark_spectra, deg_bark_spectra
        )

        # Per-frame distortion
        audible_power_ref = self.compute_audible_power(ref_bark_spectra, 1.0)
        wd_frame, wda_frame = self.per_frame_distortion(
            sym_d, asym_d, audible_power_ref
        )
    ...
    def per_frame_distortion(self, sym_d, asym_d, total_power_ref):
        ...
        d_frame = torch.sum(
            torch.pow(sym_d * self.width_of_band_bark, 2.0) + self.EPS,
            dim=-1,
            keepdim=True,
        )
    ...
    def register_16k_constants(self):
        ...
        self.abs_thresh_power = nn.Parameter(
            tensor(abs_thresh_power), requires_grad=False
        )
        ...
        self.modified_zwicker_power = nn.Parameter(
            tensor(modif_zwicker_power), requires_grad=False
        )
        # Width of band bark
        width_of_band_bark = [
            0.157344,
        ...
        self.width_of_band_bark = nn.Parameter(
            tensor(width_of_band_bark), requires_grad=False
        )
        # Bark matrix
