...
def overlapped_windowing(tensor, window, shift):
    ...
    * General purpose function *
    ...
    overlapped_slices = tf.squeeze(
        tf.extract_image_patches(
            tensor[:, :, None, None],
            ksizes=[1, window_size, 1, 1],
            strides=[1, shift, 1, 1],
            rates=[1, 1, 1, 1],
            padding="VALID",
        )
...
def magnitude_at_standard_listening_level(spectra):
    ...
    mask[11] = (
        0.5 * 25.0 / 31.25
    )  # 11th bin represents freq. 343.75 but we need 350 (proportional value)
    mask[12:104] = 1.0
    mask[104] = 0.5
...
def magnitude_at_standard_listening_level_numpy(spectra):
    ...
    # After simplifying:
    Pow = (
        Pow_correc_factor * np.mean(np.multiply(spectra, mask)) * (Nf + 2.0) / (Nf * Nf)
    )
    # Alternatively:
...
def compute_audible_power(bark_spectra, factor=1.0):
    ...
    return tf.reduce_sum(
        tf.where(cond, bark_spectra, tf.zeros(tf.shape(bark_spectra))), axis=1
    )
...
def bark_frequency_equalization(bark_spectra_ref, bark_spectra_deg):
    ...
    ref_thresholded = tf.where(
        tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0),
        bark_spectra_ref,
        tf.zeros(tf.shape(bark_spectra_ref)),
    )
    deg_thresholded = tf.where(
        tf.greater_equal(bark_spectra_deg, abs_thresh_power * 100.0),
        bark_spectra_deg,
        tf.zeros(tf.shape(bark_spectra_deg)),
    )

    # Average Pow per bark bin in reference and degraded signals
    avg_pow_per_bark_ref = tf.reduce_sum(
        tf.where(not_silent, ref_thresholded, tf.zeros(tf.shape(bark_spectra_ref))),
        axis=0,
    )
    avg_pow_per_bark_deg = tf.reduce_sum(
        tf.where(not_silent, deg_thresholded, tf.zeros(tf.shape(bark_spectra_deg))),
        axis=0,
    )
...
def loudness_computation(bark_spectra):
    ...
    """
    loudness_dens = Sl * tf.multiply(
        tf.pow(abs_thresh_power / 0.5, modified_zwicker_power),
        tf.pow(0.5 + 0.5 * bark_spectra / abs_thresh_power, modified_zwicker_power)
        - 1.0,
    )

    cond = tf.less(bark_spectra, abs_thresh_power)
    loudness_dens_limited = tf.where(
        cond, tf.zeros(tf.shape(loudness_dens)), loudness_dens
    )
...
def per_frame_distortion(D, DA, total_power_ref):
    ...
    D_frame = (
        tf.sqrt(tf.reduce_sum(tf.square(tf.multiply(D, width_of_band_bark)), axis=1))
        * sqrt_total_width
    )
...
def per_frame_PMSQE(ref_spectra, deg_spectra, alpha=0.1):
    ...
    if bark_equalization:
        if on_degraded:
            deg_bark_spectra = bark_frequency_equalization_on_degraded(
                ref_bark_spectra, deg_bark_spectra
            )
            ref_bark_spectra_corrected = ref_bark_spectra
        else:
            ref_bark_spectra_corrected = bark_frequency_equalization(
                ref_bark_spectra, deg_bark_spectra
            )
    else:
        ref_bark_spectra_corrected = ref_bark_spectra
    ...
    if degraded_gain_correction:
        deg_bark_spectra_corrected = bark_gain_equalization(
            ref_bark_spectra_corrected, deg_bark_spectra
        )
    else:
        deg_bark_spectra_corrected = deg_bark_spectra

    # Distortion matrix computation from Bark spectra:
    D, DA = compute_distortion_tensors(
        ref_bark_spectra_corrected, deg_bark_spectra_corrected
    )
...
def init_constants(
    Fs=8000,
    Pow_factor=perceptual_constants.Pow_correc_factor_Hann,
    ...
    if Fs == 16000:
        ...
        abs_thresh_power = tf.constant(
            perceptual_constants.abs_thresh_power_16k, dtype=tf.float32
        )
        modified_zwicker_power = tf.constant(
            perceptual_constants.modified_zwicker_power_16k, dtype=tf.float32
        )
        width_of_band_bark = tf.constant(
            perceptual_constants.width_of_band_bark_16k, dtype=tf.float32
        )
        sqrt_total_width = tf.constant(
            np.sqrt(np.sum(perceptual_constants.width_of_band_bark_16k)),
            dtype=tf.float32,
        )
        # Matrices
        ...
        Hann_Window = tf.constant(scipy.signal.hann(Nf, False), dtype=tf.float32)
        abs_thresh_power = tf.constant(
            perceptual_constants.abs_thresh_power_8k, dtype=tf.float32
        )
        modified_zwicker_power = tf.constant(
            perceptual_constants.modified_zwicker_power_8k, dtype=tf.float32
        )
        width_of_band_bark = tf.constant(
            perceptual_constants.width_of_band_bark_8k, dtype=tf.float32
        )
        sqrt_total_width = tf.constant(
            np.sqrt(np.sum(perceptual_constants.width_of_band_bark_8k)),
            dtype=tf.float32,
        )
        # Matrices
...
if __name__ == "__main__":
    ...
    test_PMSQE("~/dataset/sig/test/0_mic.wav", "")
