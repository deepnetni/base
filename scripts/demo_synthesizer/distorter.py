...
class Distorter:
    def __init__(
        self,
        distortion_sampling_params: Dict[str, Any] = None,
    ...
    def load_default_config(self, impulse_responses: List[str] = []):
        ...
        self.distortion_sampling_params = {
            ...
            "room_mic_impulse_response_file": {
                "range": impulse_responses,
                "likelihood": ir_likelihood,
            },
    ...
    def generate_random_params(
        self, use_provided_rir: bool = False, seed: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        for param in distortion_sampling_params:
            ...
            if (
                param == "room_mic_impulse_response_file"
                and len(distortion_sampling_params[param]["range"]) == 0
            ):
                if use_provided_rir and (
                    random.random()
                    < float(distortion_sampling_params[param]["likelihood"])
                ):
            ...
            if random.random() < float(distortion_sampling_params[param]["likelihood"]):
                if (
                    param_type == bool
                ):  # Special case for booleans - if likelihood test passes, always True
                    new_params[param] = True
                elif (
                    param_type == str
                ):  # For string types, the range should be a list of possible strings
                    new_params[param] = random.choice(
                        distortion_sampling_params[param]["range"]
                    )
        ...
        if (
            new_params.get("gsm_transmission_enabled") is not None
            and new_params["gsm_transmission_enabled"] == True
        ):
        ...
        if (
            new_params.get("room_bad_mic_response") is not None
            and new_params["room_bad_mic_response"] == True
        ):
            ...
            for mic_param in bad_mic_params:
                bad_mic_params[mic_param] = tuple(
                    [float(x) for x in bad_mic_params[mic_param]]
                )
            new_params["room_bad_mic_params"] = {
                ...
                "peak_hz": [
                    random.uniform(*bad_mic_params["peak_hz"])
                    for _ in range(peak_count)
                ],
                "peak_db": [
                    random.uniform(*bad_mic_params["peak_db"])
                    for _ in range(peak_count)
                ],
            }
        return new_params
    ...
    def generate_bad_mic_response(
        self, bad_mic_params: Dict[str, Any]
    ) -> List[IIRFilter]:
        """
        ...
        response_modules = [
            LowShelfFilter(
                cutoff_frequency_hz=bad_mic_params["low_shelf_hz"],
                gain_db=bad_mic_params["low_shelf_db"],
            ),
            HighShelfFilter(
                cutoff_frequency_hz=bad_mic_params["high_shelf_hz"],
                gain_db=bad_mic_params["high_shelf_db"],
            ),
        ]
        for cutoff, gain in zip(bad_mic_params["peak_hz"], bad_mic_params["peak_db"]):
            response_modules.append(
                PeakFilter(cutoff_frequency_hz=cutoff, gain_db=gain, q=5.0)
            )
        return response_modules

    def pedalboard_process(
        self,
        audio: np.ndarray,
        sample_rate_hz: int,
        pedalboard_params: Dict[str, Any],
        ...
        #     room_reverb = Reverb(
        #         room_size=pedalboard_params["room_freeverb_room_size"],
        #         wet_level=wet_level,
        #         dry_level=dry_level,
        #     )
        #     module_list.append(room_reverb)
        if (
            pedalboard_params.get("room_bad_mic_response") is not None
            and pedalboard_params["room_bad_mic_response"] == True
        ):
            module_list.extend(
                self.generate_bad_mic_response(pedalboard_params["room_bad_mic_params"])
            )
        if pedalboard_params.get("room_mic_impulse_response_file") is not None:
            if pedalboard_params["room_mic_impulse_response_file"] == "PROVIDED_RIR":
                with RIRFifo(
                    provided_rir["data"], provided_rir["sample_rate"]
                ) as rir_fifo_name:
                    room_mic_impulse = Convolution(
                        impulse_response_filename=rir_fifo_name
                    )
            else:
                room_mic_impulse = self.get_impulse_response_conv(
                    pedalboard_params["room_mic_impulse_response_file"]
                )
            module_list.append(room_mic_impulse)

        # Analog-digital-conversion
        if pedalboard_params.get("dac_highpass_cutoff_hz") is not None:
            dac_highpass = HighpassFilter(
                cutoff_frequency_hz=pedalboard_params["dac_highpass_cutoff_hz"]
            )
            module_list.append(dac_highpass)
        if pedalboard_params.get("dac_lowpass_cutoff_hz") is not None:
            dac_lowpass = LowpassFilter(
                cutoff_frequency_hz=pedalboard_params["dac_lowpass_cutoff_hz"]
            )
        ...
        if pedalboard_params.get("agc_expand_ratio") is not None:
            ...
            agc_expander = NoiseGate(
                threshold_db=threshold_db, ratio=pedalboard_params["agc_expand_ratio"]
            )
        ...
        if False:
            ...
            if (
                pedalboard_params.get("prepro_hardlimit_enabled") is not None
                and pedalboard_params["prepro_hardlimit_enabled"] == True
            ):
            ...
            if pedalboard_params.get("prepro_sample_rate_hz") is not None:
                if (
                    pedalboard_params.get("gsm_transmission_enabled") is not None
                    and pedalboard_params["gsm_transmission_enabled"] == True
                ):
                    assert (
                        False
                    ), "Choose either resampling on its own, or as part of GSM transmission"
                if pedalboard_params.get("prepro_or_gsm_resample_quality") is None:
                    assert False, "Missing prepro_or_gsm_resample_quality"
                prepro_resample = Resample(
                    target_sample_rate=pedalboard_params["prepro_sample_rate_hz"],
                    quality=getattr(
                        Resample.Quality,
                        pedalboard_params["prepro_or_gsm_resample_quality"],
                    ),
                )
            ...
            if pedalboard_params.get("extra_phaser_rate_hz") is not None:
                ...
                extra_phasedistortion = Phaser(
                    rate_hz=pedalboard_params["extra_phaser_rate_hz"], mix=mix
                )
                module_list.append(extra_phasedistortion)
            if pedalboard_params.get("extra_tanh_distortion_drive_db") is not None:
                extra_nonlinear = Distortion(
                    drive_db=pedalboard_params["extra_tanh_distortion_drive_db"]
                )
        ...
        if pedalboard_params.get("microaug_shift_ms") is not None:
            ...
            microaug_shift = Delay(
                delay_seconds=pedalboard_params["microaug_shift_ms"] / 1000.0, mix=mix
            )
        ...
        if (
            ...
            if pedalboard_params.get("prepro_or_gsm_resample_quality") is None:
                assert False, "Missing prepro_or_gsm_resample_quality"
            transmission_gsm = GSMFullRateCompressor(
                quality=getattr(
                    Resample.Quality,
                    pedalboard_params["prepro_or_gsm_resample_quality"],
                )
    ...
    def apply_distortions(
        ...
        if params is None:
            if self.seed_mode == "random":
                params = self.generate_random_params(
                    use_provided_rir=(provided_rir is not None)
                )
                ...
                if self.seed_mode == "fixed_iter":
                    ...
                    seed = seed + seed_iter
                params = self.generate_random_params(
                    use_provided_rir=(provided_rir is not None), seed=seed
                )
            else:
        ...
        return (
            self.pedalboard_process(audio, sample_rate_hz, params, provided_rir),
            params,
        )
